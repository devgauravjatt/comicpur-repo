...
// add categories
categoriesRouter.post('/', reqValidator('json', addCategoriesBodySchema, true), async (c) => {
...
// update categories
categoriesRouter.put('/', reqValidator('json', updateCategoriesBodySchema, true), async (c) => {
	const body = c.req.valid('json');
	try {
